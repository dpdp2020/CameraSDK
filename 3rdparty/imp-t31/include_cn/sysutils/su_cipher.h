...
/**
 * DES加密算法专用
 * @remarks 支持选择SDES，TDES
 */
typedef enum IN_UNF_CIPHER_DES_ALG_E
{
	IN_UNF_CIPHER_DES_SDES_ALG = 0x0,
	IN_UNF_CIPHER_DES_TDES_ALG = 0x1,
	IN_UNF_CIPHER_DES_OTHER = 0x2
} IN_UNF_CIPHER_DES_ALG;
...
typedef struct IN_UNF_CIPHER_CTRL_S
{
	...
	IN_UNF_CIPHER_DES_ALG enDesAlg;		/**< DES算法专用模式*/
} IN_UNF_CIPHER_CTRL;
/**
 * @fn int SU_CIPHER_Malloc_Buffer(unsigned int size)
 * 为准备加密、解密的数据申请缓存buffer.
 * @param size 缓存buffer申请字节数
 * @retval 0 成功，非0失败
 * @attention 必须在进行加密、解密之前申请.size要大于等于一次性加密数据的大小
 *
 *
 * */
int SU_CIPHER_Malloc_Buffer(unsigned int size);
