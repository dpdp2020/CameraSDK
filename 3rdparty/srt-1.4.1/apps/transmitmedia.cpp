...
unsigned long transmit_bw_report = 1;
unsigned long transmit_stats_report = 1;
...
void SrtCommon::InitParameters(string host, map<string,string> par)
{
    ...
    {
        m_timeout = atoi(par.at("timeout").c_str());	//stoi(par.at("timeout"), 0, 0);
        par.erase("timeout");
    }
    ...
    if (par.count("port"))
    {
        m_outgoing_port = atoi(par.at("port").c_str());	//stoi(par.at("port"), 0, 0);
        par.erase("port");
    }
...
void SrtCommon::ConnectClient(string host, int port)
{
    ...
    //srt_log(SRT_LOG_DEBUG,"prepare Connecting to %s:%d\n",host.c_str(),port);
    //int conn_timeout=0;
    //int conn_timeout_len = sizeof conn_timeout;

    //将超时时间设置为5s;
    int set_conn_timeout_ms= 5000;
    result=srt_setsockopt(m_sock,0,SRTO_CONNTIMEO,&set_conn_timeout_ms,sizeof set_conn_timeout_ms);
    if(result==-1)
    {
        //MY_LOG(D_ALL,"set SRTO_CONNTIMEO failed");
    }
    //srt_getsockopt(m_sock,0,SRTO_CONNTIMEO,&conn_timeout,&conn_timeout_len);

    
    //srt_log(SRT_LOG_DEBUG, "SRTO_CONNECTIME: %d \n", conn_timeout);
    //bool m_blocking_mode=false;
    //int m_blocking_mode_len=0;
    //srt_getsockopt(m_sock,0,SRTO_RCVSYN, &m_blocking_mode, &m_blocking_mode_len);
     
    //srt_log(SRT_LOG_TRACE,"default blocking_mode: %d\n",m_blocking_mode);
    //将srt_connect 更改为阻塞模式，默认超时等待3s
    bool yes=true;
    result=srt_setsockopt(m_sock, 0, SRTO_RCVSYN, &yes, sizeof yes);
    if(result==-1)
    {
     
        //MY_LOG(D_ALL,"set SRTO_RCVSYN true failed");
    }

    //cout <<"[wjj] after set m_blocking_mode (m_blcking_mode_len)"<<m_blocking_mode<<" " <<m_blocking_mode_len<<endl;
    //srt_log(SRT_LOG_DEBUG, "now m_blocking_mode : %d \n",m_blocking_mode);
...
int SrtTarget::Write(const char* data, size_t size, ostream &out_stats)
{
    ...
    //parse_ts_data((uint8_t*)data, (int)size);
...
protected:
    ...
    void Setup(string host, int port, map<string,string> attr)
    {
        ...
        if (attr.count("ttl"))
        {
            int ttl = atoi(attr.at("ttl").c_str());	//stoi(attr.at("ttl"));
